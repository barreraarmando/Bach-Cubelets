import sys
import random
from automaton import machines, converters
...
probabilidadesR = [["negra", 50], ["blanca", 25], ["corchea", 25]]
numCompases = 8
numNotas = 32;
randomMelody = [];
...
def construirArmonia(numCompases, grados, tipo):
    gradoAnterior = 0
    nombresGrados = ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII']
    eventos = ['to_I', 'to_II', 'to_III', 'to_IV', 'to_V', 'to_VI', 'to_VII']
    probabilidadesM = [
        [10,15,15,20,20,15,5],
        [10,5,10,20,30,20,5],
        [20,15,5,15,10,30,5],
        [20,15,10,10,20,15,10],
        [30,5,10,15,10,20,10],
        [20,25,10,20,15,5,5],
        [100,0,0,0,0,0,0]
    ]
    ...
    if tipo == "major":
        probabilidades = probabilidadesM
    else:
        probabilidades = probabilidadesm
    
    for i in range(numCompases):
        moneda = random.randint(0, 100)
        acumulador = 0
        grado = 0
        for cont, p in enumerate(probabilidades[gradoAnterior]):
            if (moneda >= acumulador and moneda <= (acumulador + p)):
                grado = cont
                break
            acumulador = acumulador + p
        armonia.append(eventos[grado])
        gradoAnterior = grado
...
def main(argv):        
    ...
    def direccionarGrado(new_state, triggered_event):
        ...
        if new_state == 'V':
            generarMelodia(grados[4], tonalidad, escala, tipo, 'V')
        if new_state == 'VI':
    ...
    events = construirArmonia(numCompases-3, grados, tipo)
